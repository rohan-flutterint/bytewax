...
use crate::errors::ByteErr;
use crate::execution::FLAG;
...
add_pymethods!(
    ...
    args { epoch_length: Duration => Duration::zero() }
);
                ...
                caps = if let Some((output_cap, change_cap)) = caps.clone() {
                    ...
                    if !probe.less_than(epoch) {
                        match parts.next() {
                            ...
                            err => {
                                panic =
                                    Some(err.pyerr::<PyRuntimeError>("Error while getting input"));
                            }
                        }
                    }
